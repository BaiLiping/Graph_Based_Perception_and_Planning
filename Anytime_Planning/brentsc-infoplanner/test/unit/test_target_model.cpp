...
TEST_CASE("Target Motion Model Add and Remove", "[TargetMotionMode]") {
  std::vector<nx::Target> test_targets;
  ...
  for (int i = 0; i < 5; i++) {
    Vector2d y;
    y << i, i;
    test_targets.push_back(nx::Target(i, y, A, W));
  }

  SECTION("Testing Empty Target Model") {

    nx::TargetModel tmm(map, cmap);
